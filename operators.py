...
class operators():
    ...
    def multi_fractional(self, p, pm_1, lr, epsilon=1e-9):
        ...
        if 1 - self.alpha2 > 0 :
            ...
            t2 = (1 / torch.exp(torch.lgamma(torch.tensor(3 - self.alpha2))))  * second_order_grad.detach() * torch.abs(p.data.detach() - pm_1.data.detach()) ** (2 - self.alpha2)
    ...
    def distributed_fractional(self, p, pm_1, lr, epsilon=1e-9):
        ...
        integral_right = lambda alpha: (1 / (torch.exp(torch.lgamma(torch.tensor(3 - alpha))))) * second_order_grad.detach() * torch.abs(p.data.detach() - pm_1.data.detach()) ** (2 - alpha)
        delta = integral_left(self.alpha1)
        ...
        for n in range(1, self.N):
            ...
            if (1 - (self.alpha1 + n * d_alpha)) > 0 :
                BOBO = integral_left(self.alpha1 + n * d_alpha)
                print(n, torch.mean(BOBO))
            else:
                
                # if second_order_grad.sum == 0:
                #     second_order_grad = epsilon
                BOBO = integral_right(self.alpha1 + n * d_alpha)
        ...
        BOBO = integral_right(self.alpha2)
        # if torch.any(torch.isnan(BOBO)) or torch.any(torch.isinf(BOBO)):
        #     BOBO = torch.where(torch.isnan(BOBO), torch.tensor(epsilon), BOBO)
        #     BOBO = torch.where(torch.isinf(BOBO), torch.tensor(epsilon), BOBO)
